...
int main(void){
    ...
    int i; /* counter for initializing elements 0-9999 of array a */
    ...
    printf("Enter number between 0 and 210:\n");
    scanf("%d" , &key);
...
int binarySearch(const int b[] , int searchKey , int low , int high){
    ...
    while (low <= high){
        /* detetmine middle element of subarray being searched */
        middle = (low + high) / 2;
...
void printRow(const int b[] , int low , int mid , int high){
    ...
    for (i = 0; i < SIZE; i++){
        ...
        if (i < low || i > high){
            ...
            printf("%4d*" , b[i]); /* mark middle value */
        } /* end else if */
